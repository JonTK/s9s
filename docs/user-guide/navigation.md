...
- Multi-pane navigation with `Ctrl+W` bindings
- Commands: `:split`, `:vsplit`, `:close`, `:only`

### Bookmarks (Planned)
- Set bookmarks with `m<letter>`
- Jump to bookmarks with `'<letter>`
- Commands: `:marks`, `:delmarks`

### Macros (Planned)
- Record macros with `q<letter>`
- Play macros with `@<letter>`
- Commands: `:reg` to list macros

### Command Mode Extensions (Planned)
- Extended `:set` commands for customization
- `:source` for loading command files
