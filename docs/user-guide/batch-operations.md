...
S9S currently supports the following batch operations on selected jobs:
...
- **Priority Changes**: Batch modification of job priorities
- **Node Operations**: Batch drain, resume, or maintenance of nodes
- **Time Limit Changes**: Modifying time limits for running jobs
- **Partition Changes**: Moving pending jobs between partitions
- **Conditional Operations**: Complex filter-based automated actions
- **Scheduled Operations**: Recurring batch operation scheduling
- **Template Operations**: Saved batch operation templates
- **Rollback/Undo**: Undoing batch operations (`:undo`, `:rollback`)

### Operation Scope

- Batch operations only work on jobs, not nodes or other resources
- Operations are executed sequentially, not in parallel
- Large batch operations may take time to complete
- Export operations generate files locally (no direct remote export)
